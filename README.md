...
- **Pending Market List** — upload a "pending deliveries" Excel export. It's split by supplier, cross-checked against your supplier directory for an email, and turned into one email per supplier containing only their own rows as an HTML table. For multi-property operations, each supplier's rows are further grouped and labeled by property based on the PO number prefix — see `IS_CLUSTER` under [Environment variables](#environment-variables).
